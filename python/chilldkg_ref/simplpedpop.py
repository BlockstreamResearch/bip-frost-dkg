...
def pop_prove(seckey: bytes, idx: int, random: bytes) -> Pop:
    sig = schnorr_sign(pop_msg(idx), seckey, aux_rand=random, tag_prefix=POP_MSG_TAG)
    return Pop(sig)
...
def participant_step1(
    seed: bytes, t: int, n: int, idx: int, random: bytes
) -> Tuple[
    ParticipantState,
    bytes,
    ...
    pop = pop_prove(vss.secret().to_bytes(), idx, random)
